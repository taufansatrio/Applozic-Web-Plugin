	...
	// var mckGroupService = new MckGroupService();
			...
			_this.init = function() {
				...
				$mck_text_box
						.keydown(function(e) {
							...
							if (e.keyCode === 13 && (e.shiftKey || e.ctrlKey)) {
								...
								mckInitializeChannel.sendTypingStatus(1,
										mckMessageLayout.getMckMessageInner()
												.data('mck-id'));
							}
						});
				$applozic(d)
						...
						.on(
								...
								function(e) {
									...
									tabId = (typeof tabId !== "undefined" && tabId !== "") ? tabId
											.toString()
											: "";
									var userName = $applozic(this).data(
											"mck-name");
									userName = (typeof userName !== "undefined" && userName !== "") ? userName
											.toString()
											: "";
									var supportId = $applozic(this).data(
											"mck-supportid");
						...
						.on(
								...
								function(e) {
									...
									topicId = (typeof topicId !== "undefined" && topicId !== "") ? topicId
											.toString()
											: "";
									var topicStatus = $applozic(this).data(
											"mck-topic-status");
									if (topicStatus) {
						...
						.bind(
								...
								function() {
									if ($mck_contacts_inner.scrollTop()
											+ $mck_contacts_inner.innerHeight() >= $mck_contacts_inner[0].scrollHeight) {
						...
						.on(
								...
								function(e) {
									...
									var isBlock = !$mck_msg_inner
											.data('blocked');
									if (isGroup) {
										$li_mck_block_user.removeClass('vis')
												.addClass('n-vis');
										return;
									}
									...
									if (confirm(blockText)) {
										mckContactService.blockUser(tabId,
												isBlock);
									}
								});
				$mck_leave_group_btn.on('click', function(e) {
					...
					if (confirm('Are you sure want to exit this group!')) {
						mckGroupService.leaveGroup({
							'groupId' : tabId, 'apzCallback' : mckGroupLayout.onGroupLeft
						});
					}
				});
				...
				$applozic(d)
						.on(
								...
								function(e) {
									e.preventDefault();
									var userId = $applozic(this).parents(
											'.mck-li-group-member').data(
											'mck-id');
						...
						.on(
								...
								function(e) {
									var conversationId = $applozic(this).data(
											'mck-conversationid');
									var priceText = $applozic(this).data(
											'mck-topic-price');
									if (typeof (MCK_PRICE_DETAIL) === 'function'
											&& priceText && conversationId) {
										var conversationPxy = MCK_CONVERSATION_MAP[conversationId];
				...
				$mck_msg_form
						.submit(function(e) {
							...
							$mck_msg_response.removeClass('vis').addClass(
									'n-vis');
				...
				$applozic(d)
						.bind(
								"click",
								function(e) {
									$applozic(".mck-context-menu").removeClass(
											"vis").addClass("n-vis");
									...
									$applozic('.mcktypeahead.mck-dropdown-menu')
											.hide();
								});
			};
								...
								success : function(data) {
									if (data === 'success') {
										...
										if (currentTabId === tabId) {
											...
											var $latestMessageDiv = $mck_msg_inner
													.children("div[name='message']:last");
											if ($latestMessageDiv.length > 0) {
												mckMessageService
														.updateContactList(
																tabId, isGroup);
											} else {
												var contact = (isGroup) ? mckGroupUtils
														.getGroup(tabId)
														: mckMessageLayout
																.getContact(tabId);
					...
					$applozic
							.ajax({
								...
								success : function() {
									...
									var contact = (isGroup) ? mckGroupUtils
											.getGroup(tabId) : mckMessageLayout
											.getContact(tabId);
									var contHtmlExpr = (contact.isGroup) ? 'group-'
											+ contact.htmlId
											: 'user-' + contact.htmlId;
							...
							success : function(data) {
								...
								if (individual) {
									...
									if (data + '' === "null"
											...
											|| data.message.length === 0) {
										if (params.startTime) {
											$mck_show_more_icon.removeClass(
													'n-vis').addClass('vis');
											$mck_show_more_icon
													.fadeOut(
															3000,
		...
		function MckMessageLayout() {
			...
			var contactbox = '<li id="li-${contHtmlExpr}" class="person ${contIdExpr}" data-mck-id="${contIdExpr}" data-isgroup="${contTabExpr}" data-mck-conversationid="${conversationExpr}" data-msg-time="${msgCreatedAtTimeExpr}">'
					...
					+ '<span class="mck-cont-msg-wrapper preview msgTextExpr"></span>'
			...
			_this.loadTab = function(params, callback) {
				...
				if (params.tabId) {
					if ($applozic('.person[data-mck-id ="' + params.tabId
							+ '"]').length == 0) {
						_this.updateRecentConversationList(
								params.isGroup ? mckGroupUtils
										.getGroup(params.tabId) : _this
										.fetchContact(params.tabId), undefined,
								true, params.prepend);
						$applozic(
								'.person[data-mck-id ="' + params.tabId + '"]')
								.addClass('active');
						$applozic(".mck-contacts-inner").scrollTop(
								$(".left .person.active").offset().top
										- $applozic(".mck-contacts-inner")
												.offset().top
										+ $applozic(".mck-contacts-inner")
												.scrollTop());
					} else {
					...
					$applozic('.chat[data-mck-id ="' + params.tabId + '"]')
							.addClass('active-chat');
				}
				if ($(".left .person.active").length) {
					$mck_msg_inner = $applozic(".mck-message-inner[data-mck-id='"
							+ params.tabId + "']");
				}
				var currTabId;
				if ($mck_msg_inner) {
				...
				if (params.tabId) {
					...
					mckInitializeChannel.subscibeToTypingChannel(params.tabId,
							params.isGroup);
					var contact = (params.isGroup) ? mckGroupUtils
							.getGroup(params.tabId) : mckMessageLayout
							.getContact(params.tabId);
					var contactHtmlExpr = (contact.isGroup) ? 'group-'
							+ contact.htmlId : 'user-' + contact.htmlId;
					$applozic(
							"#li-" + contactHtmlExpr + " .mck-unread-count-box")
			...
			_this.closeConversation = function(data) {
				...
				$mck_msg_error.removeClass('n-vis').addClass('vis').addClass(
						'mck-no-mb');
				$applozic("#mck-write-box").removeClass('vis')
						.addClass('n-vis');
			};
			...
			_this.removeConversationThread = function(tabId, isGroup) {
				$mck_msg_inner = mckMessageLayout.getMckMessageInner();
				mckStorage.clearMckMessageArray();
				var contact = (isGroup) ? mckGroupUtils.getGroup(tabId)
				...
				var contactIdExpr = (isGroup) ? 'group-' + htmlId : 'user-'
						+ htmlId;
				$applozic("#li-" + contHtmlExpr + " .mck-cont-msg-wrapper")
						.html('');
				$applozic("#li-" + contHtmlExpr + " .time").html('');
				if (currentTabId === tabId) {
					$mck_msg_inner.html("");
			...
			_this.populateMessage = function(messageType, message, notifyUser) {
				...
				(message.groupId) ? mckMessageLayout.addGroupFromMessage(
						message, true) : mckMessageLayout
						.addContactsFromMessage(message, true);
				...
				/*
				 * if ((typeof tabId === "undefined") || tabId === "") { if
				 * (messageType === "APPLOZIC_01" || messageType ===
				 * "MESSAGE_RECEIVED") { if (typeof contact === 'undefined') {
				 * contact = (message.groupId) ? mckGroupLayout
				 * .createGroup(message.groupId) : mckMessageLayout
				 * .createContact(message.to); } var ucTabId = (message.groupId) ?
				 * 'group_' + contact.contactId : 'user_' + contact.contactId;
				 * if (message.contentType !== 10) {
				 * mckMessageLayout.incrementUnreadCount(ucTabId); }
				 * mckNotificationService.notifyUser(message); var
				 * contactHtmlExpr = (message.groupId) ? 'group-' +
				 * contact.htmlId : 'user-' + contact.htmlId; $applozic( "#li-" +
				 * contactHtmlExpr + " .mck-unread-count-text").html(
				 * mckMessageLayout.getUnreadCount(ucTabId)); if
				 * (mckMessageLayout.getUnreadCount(ucTabId) > 0) { $applozic(
				 * "#li-" + contactHtmlExpr + " .mck-unread-count-box")
				 * .removeClass("n-vis").addClass("vis"); }
				 * mckMessageService.sendDeliveryUpdate(message); } } else { if
				 * (typeof contact === 'undefined') { contact =
				 * (message.groupId) ? mckGroupLayout
				 * .createGroup(message.groupId) :
				 * mckMessageLayout.createContact(message.to); } if (messageType
				 * === "APPLOZIC_01" || messageType === "MESSAGE_RECEIVED") { if
				 * (typeof contact !== 'undefined') { var isGroupTab =
				 * $mck_msg_inner.data('isgroup'); if (typeof tabId !==
				 * 'undefined' && tabId === contact.contactId && isGroupTab ===
				 * contact.isGroup) { if (message.conversationId &&
				 * (IS_MCK_TOPIC_HEADER || IS_MCK_TOPIC_BOX)) { var currConvId =
				 * $mck_msg_inner .data('mck-conversationid'); if (currConvId &&
				 * currConvId.toString() === message.conversationId .toString()) {
				 * mckMessageLayout.addMessage(message, true, true, true);
				 * mckMessageService .sendReadUpdate(message.pairedMessageKey); } }
				 * else { mckMessageLayout.addMessage(message, true, true,
				 * true); mckMessageService
				 * .sendReadUpdate(message.pairedMessageKey); } if
				 * (!message.groupId) {
				 * $applozic("#mck-tab-status").html("Online");
				 * mckUserUtils.updateUserStatus({ 'userId' : message.to,
				 * 'status' : 1 }); } // Todo: use contactNumber instead of
				 * contactId // for Google Contacts API. } else { if
				 * (message.contentType !== 10) { var ucTabId =
				 * (message.groupId) ? 'group_' + contact.contactId : 'user_' +
				 * contact.contactId; mckMessageLayout
				 * .incrementUnreadCount(ucTabId); }
				 * mckMessageService.sendDeliveryUpdate(message); } if
				 * (notifyUser) { mckNotificationService.notifyUser(message); } } }
				 * else if (messageType === "APPLOZIC_02") { if (($applozic("." +
				 * message.oldKey).length === 0 && $applozic("." +
				 * message.key).length === 0) || message.contentType === 10) {
				 * if (mckContactListLength > 0) {
				 * mckMessageLayout.addContactsFromMessage( message, true); }
				 * else { if (typeof contact !== 'undefined') { var isGroupTab =
				 * $mck_msg_inner .data('isgroup'); if (typeof tabId !==
				 * 'undefined' && tabId === contact.contactId && isGroupTab ===
				 * contact.isGroup) { mckMessageLayout.addMessage(message, true,
				 * true, true); if (message.type === 3) { $applozic( "." +
				 * message.key + " .mck-message-status") .removeClass(
				 * 'mck-icon-time') .addClass('mck-icon-sent'); mckMessageLayout
				 * .addTooltip(message.key); } } } } } } }
				 */
				$mck_loading.removeClass('vis').addClass('n-vis');
			};
		...
		function MckUserUtils() {
			...
			_this.toggleBlockUser = function(tabId, isBlocked) {
				...
				if (isBlocked) {
					...
					$mck_msg_error.removeClass('n-vis').addClass('vis')
							.addClass('mck-no-mb');
					$applozic("#mck-write-box").removeClass('vis').addClass(
							'n-vis');
					$mck_tab_title.removeClass('mck-tab-title-w-status');
					$mck_tab_status.removeClass('vis').addClass('n-vis');
					$mck_typing_box.removeClass('vis').addClass('n-vis');
					...
					$applozic("#mck-write-box").removeClass('n-vis').addClass(
							'vis');
		...
		function MckGroupLayout() {
			...
			var groupContactbox = '<li id="li-gm-${contHtmlExpr}" class="${contIdExpr} mck-li-group-member" data-mck-id="${contIdExpr}" data-alpha="${contFirstAlphaExpr}">'
			...
			$('.mck-group-name-sec div[contenteditable]')
					.keypress(function() {
					...
					.on(
							{
								'paste' : function(e) {
									var len = this.innerHTML.length, cp = e.originalEvent.clipboardData
											.getData('text');
									if (len < MAX_GROUP_NAME_SIZE)
										this.innerHTML += cp.substring(0,
												MAX_GROUP_NAME_SIZE - len);
									return false;
								},
								'drop' : function(e) {
									e.preventDefault();
			...
			$mck_group_name_save.on('click', function() {
				...
				if (groupName.length > 0) {
					...
					if (currTabId && isGroupTab) {
						$mck_group_name_edit.removeClass('n-vis').addClass(
								'vis');
						$mck_group_name_save.removeClass('vis').addClass(
								'n-vis');
			...
			_this.addMemberToGroup = function(group, userId) {
				if (typeof group.members === 'object') {
					...
					if (typeof group.removedMembersId === 'object'
							&& (group.removedMembersId.indexOf(userId) !== -1)) {
						group.removedMembersId.splice(group.removedMembersId
								.indexOf(userId), 1);
					}
			...
			_this.addGroupStatus = function(group) {
				...
				if (isGroupLeft) {
					mckGroupLayout.onGroupLeft("", {groupId:group.contactId});
					$mck_tab_title.removeClass("mck-tab-title-w-status");
					$mck_tab_status.removeClass('vis').addClass('n-vis');
				...
				if (typeof group === 'object') {
					...
					if ($mck_group_info_tab.hasClass('vis')) {
						...
						if (groupId === currGroupId) {
							var contact = mckMessageLayout.fetchContact(''
									+ userId);
							if ($applozic("#mck-group-member-list #li-gm-user-"
				...
				if (typeof group === 'object') {
					...
					if ($mck_group_info_tab.hasClass('vis')) {
						...
						if (groupId === currGroupId) {
							var contact = mckMessageLayout.fetchContact(''
									+ userId);
							var $liRemovedMember = $applozic("#li-gm-user-"
									+ contact.htmlId);
			...
			_this.onUpdateGroupInfo = function(response, params) {
				...
				if (typeof group === 'object') {
					...
					if ($applozic("#li-group-" + group.htmlId).length > 0) {
						$applozic("#li-group-" + group.htmlId + " .name").html(
								group.displayName);
						if (group.imageUrl) {
							$applozic(
									"#li-group-" + group.htmlId + " .blk-lg-3")
									.html("<img src=' " + group.imageUrl + "'>");
						}
					}
					MCK_GROUP_MAP[group.contactId] = group;
				}
			...
			_this.onGroupFeed = function(response, params) {
				...
				if (response.status === 'success') {
					...
					if (params.isMessage && typeof params.message === 'object') {
						mckMessageLayout.populateMessage(params.messageType,
								params.message, params.notifyUser);
					}
		...
		function MckNotificationService() {
			...
			_this.notifyUser = function(message) {
				...
				var contact = (message.groupId) ? mckGroupUtils.getGroup(''
		...
		function MckInitializeChannel($this) {
			...
			_this.onMessage = function(obj) {
				...
				if (messageType === "APPLOZIC_04"
					...
					$applozic(
							"#li-user-" + contact.htmlId
									+ " .mck-unread-count-box").removeClass(
							"vis").addClass("n-vis");
